...
const SmartDevices = () => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch("http://localhost:5000/getProducts?category=smart-devices");
      if (response.ok) {
        const result = await response.json();
        setProducts(result);
      } else {
        console.error("Failed to fetch smart devices");
      }
    } catch (error) {
  ...
  return (
    <>
      ...
      <div className="container my-5">
        <h2 className="text-center mb-4">Smart Devices</h2>
        <div className="row">
          {products.length > 0 ? (
            ...
            <p className="text-center">No smart devices available.</p>
          )}
        </div>
      </div>
      <Footer />
