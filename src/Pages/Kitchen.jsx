...
const Kitchen = () => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch("http://localhost:5000/getProducts?category=kitchen");
      if (response.ok) {
        const result = await response.json();
        setProducts(result);
      } else {
        console.error("Failed to fetch kitchen products");
      }
    } catch (error) {
  ...
  return (
    <>
      ...
      <div className="container my-5">
        <h2 className="text-center mb-4">Kitchen Products</h2>
        <div className="row">
          {products.length > 0 ? (
            ...
            <p className="text-center">No kitchen products available.</p>
          )}
        </div>
      </div>
      <Footer />
